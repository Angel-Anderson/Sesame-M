 ...
 * 模块 hook 业务逻辑核心。
 * <p>
 * 本类不继承任何框架类、不直接引用 libxposed / de.robv Xposed API，
 * 以保证在 LSPosed（{@link LibXposedEntry}）与 LSPatch / NPatch（{@link LegacyXposedEntry}）
 * 两条入口路径下都能被安全加载。框架差异通过 {@link HookEngine} 与
 * {@link io.github.aw1y2z.sesame.util.XHelpers} 桥接。
 */
public class ApplicationHook {
    ...
    public static void onModuleLoadedInternal() {
        XHelpers.log(4, TAG, "event=module_loaded api=" + XHelpers.getApiVersion()
                + " framework=" + XHelpers.getFrameworkName() + " version=" + XHelpers.getFrameworkVersion());
        markFile("/sdcard/sesame_diag.txt", "onModuleLoaded " + XHelpers.getFrameworkName() + " api=" + XHelpers.getApiVersion());
        try {
    ...
    public static void onPackageReadyInternal(String packageName, ClassLoader classLoader) {
            ...
            try {
                XHelpers.findAndHookMethod("android.app.Service", classLoader, "onCreate", new XC_MethodHook() {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        ...
                        LibraryUtil.loadLibSesame(context);
                        service = appService;
    ...
    private synchronized static Boolean initHandler(Boolean force) {
        if (service == null) {
            return false;
        }
    ...
    // 滑块验证hook注册
    private static void initSimplePageManager() {
     ...
     */
    private static boolean shouldEnableSimplePageManager() {
